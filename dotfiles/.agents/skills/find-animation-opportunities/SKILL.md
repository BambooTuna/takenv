...
Every "Suggested motion" cell carries exact values — the curve, the duration, the properties — pulled from this repo's shared vocabulary (`--ease-out: cubic-bezier(0.23, 1, 0.32, 1)`, `--ease-in-out: cubic-bezier(0.77, 0, 0.175, 1)`, `--ease-drawer: cubic-bezier(0.32, 0.72, 0, 1)`), adjusted to the interaction and existing tokens. Prefer `transform` and `opacity`; evaluate layout or clip-path animation in context. Include reduced-motion handling (instant or gentle feedback) and `@media (hover: hover) and (pointer: fine)` gating when the suggestion involves hover.
