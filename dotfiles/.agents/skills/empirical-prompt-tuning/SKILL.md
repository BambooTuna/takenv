...
使わない場面: 一回限りの使い捨てプロンプト（評価コストが割に合わない）、成功率の改善ではなく書き手の主観的好みを反映したいだけのとき

## ワークフロー

0. **Iteration 0 — description と body の整合チェック**（静的、dispatch 不要）
   - frontmatter `description` が謳う trigger / 用途と body がカバーする範囲を照らし合わせ、乖離があれば iter 1 前に揃える
   - 例: description「navigation / form filling / data extraction」だが body は `npx playwright test` の CLI ref のみ、のような乖離
   - 飛ばすと subagent が description に合わせて body を「再解釈」し、要件未達なのに精度が出る（false positive）
...
2. **バイアス排除読み**: 指示を「白紙」の実行者に読ませる。Task tool で **新規 subagent を dispatch** する（自己再読は不可。理由は後述「Red flags」表）。並列で複数シナリオを同時実行する場合は単一メッセージ内で複数 Agent 呼び出しを並べる。dispatch 不能環境の扱いは「環境制約」節を参照。
...
**`tool_uses` の質的解釈**: 精度だけ見ると skill の問題が隠れる。シナリオ間で他シナリオ比 **3-5 倍以上** なら、その skill は decision-tree index 寄りで自己完結性が低いサイン（recipe が skill 内に無く、実行者が references/ を横断探索している）。精度 100% でもこの偏りがあれば iter 2 発動の根拠になる。対処: 「最小完成例 inline」を SKILL.md 冒頭に追加すると大幅低下する。

**修正の波及パターン**: 事前見積もりと実測は次の 3 パターンでズレうる — 保守的に振れる（1 修正で複数軸狙ったが 1 軸しか動かず、複数軸狙いは外しがち）、上振れ（コマンド + 設定 + 期待出力の組合せのような 1 つの構造的情報が複数軸の判定文言を同時に満たす）、ゼロ振れ（軸名から推測した修正が判定文言のどれにも届かない）。安定させるには差分適用前に「この修正が判定文言のどれを満たすか」を subagent に言語化させる。閾値文言レベルで紐付けないと見積もり精度が出ない。
...
新規 subagent を dispatch できない環境（既に subagent として動作している、Task tool が無効化されている等）では、本 skill は **適用しない**（自己再読で代替するのは NG。バイアスが入り評価結果を信じられない）。代替: 親セッションのユーザーに別セッションでの評価を依頼するか、「empirical evaluation skipped: dispatch unavailable」と明示報告する。**構造審査モード**（実行評価ではなく記述の整合性・明瞭性だけを見たい場合）: subagent への依頼プロンプトに「今回は構造審査モード: 実行ではなくテキスト整合性チェック」と明記すれば skip 動作を回避し静的レビューを返せるが、empirical の代替ではなく補助（連続クリア判定には使えない）。
...
## Red flags / よくある失敗（合理化に注意）

| 出てくる合理化・失敗                                  | 実態                                                                                                                 |
| ----------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| 「自分で読み直せば同じ効果がある」                    | 直前に書いた文章を "客観視" はできない。必ず新規 subagent を dispatch する。                                         |
| 「1 シナリオで充分」                                  | 1 シナリオは過適合する。最低 2、できれば 3（中央値 1 + edge 1〜2）。                                                 |
| シナリオが楽すぎる / 難しすぎる                       | どちらもシグナルが出ない。中央値 1 つ + edge 1 つの構成に戻す。                                                     |
| 「不明瞭点ゼロが 1 回出たから終わり」                 | 偶然なこともある。連続 2 回で確定判定。                                                                              |
| 「複数の不明瞭点を一気に潰そう」/変更が多すぎる       | 何が効いたか分からなくなる。1 イテレーション 1 テーマ。                                                              |
| 「関連する微修正も純粋に 1 件ずつ別 iter に分けよう」 | 逆方向の罠。"1 テーマ" は意味単位。関連する 2-3 件の微修正は 1 iter にまとめて良い。分けすぎると iter 数が爆発する。 |
| 「メトリクスが良いから質的フィードバックは無視」      | 時間短縮しか追わないと重要な説明が削られて脆くなる。質的を主に。                                                     |
