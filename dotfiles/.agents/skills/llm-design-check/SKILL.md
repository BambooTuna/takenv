...
- **生成と検証の契約を揃える。** 表現可能な整合条件はschemaへ、参照や原文の照合は後処理へ置く。検証方法と再試行は精度・費用・失敗時の影響に合わせる（checklist §7参照）。
...
- **根拠付け方式はタスク性質で選ぶ。** quote自体が成果物なら原文スパンを保存し、参照導線なら行番号/IDと実在性を検証する。根拠の関連性は一致検査だけでは保証できないため、必要な精度に応じて独立レビューなどを追加する。
