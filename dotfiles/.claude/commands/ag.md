...
If you already know your AGENT and TEAMS from a previous `/ag` call in this session, skip to **Execute** below.

Otherwise, run: `~/.agents/skills/ag/scripts/whoami.sh "$(pwd)" claude-code`
...
→ Show the user the available teams from the output, then:
  ...
  3. **You MUST use join.sh** — run: `~/.agents/skills/ag/scripts/join.sh <team> <agent_name> claude-code "$(pwd)"`
  4. Show the result and explain:

  > **Joined!** You can now use `/ag` to check and send messages.
  > - `/ag` — check inbox
  > - `/ag send <agent> <message>` — send a message
  > - `/ag team` — list team members
  > - `/ag history` — message history
  > - `/ag mode <monitor|turn|both|off>` — switch delivery mode
  > - `/ag actas <name>` — switch to another role in this project (creates if needed)
  > - `/ag drop <name>` — remove a role from this project
  > - `/ag spawn <type> <name>` — launch a new agent in a tmux pane / terminal and have it actas <name>
  > - `/ag despawn <name>` — tear down a member you spawned (graceful, or `--force`)
  ...
  5. **REQUIRED — Do NOT skip this step.** Ask the user to pick a delivery mode using exactly this prompt:
     ...
     - Map the chosen number to a mode and run:
       `~/.agents/skills/ag/scripts/delivery.sh set <mode> claude-code "$(pwd)"`
...
→ No exact registration exists for this project, but there are same-type agent names registered elsewhere.
  ...
  4. Run: `~/.agents/skills/ag/scripts/join.sh <team> <agent_name> claude-code "$(pwd)"`
  5. Then continue with the normal post-join flow above.

## Execute

**Only use scripts in `~/.agents/skills/ag/scripts/` — do not read or modify files under `teams/` or `db/` directly.**

**Ensure monitor is running first.** Before processing any subcommand below, check whether this session already has an `agmsg inbox stream` Monitor task in its TaskList. If not, and the project's delivery mode is `monitor` or `both` (check via `~/.agents/skills/ag/scripts/delivery.sh status claude-code "$(pwd)"`), invoke the Monitor tool now:

- command: `~/.agents/skills/ag/scripts/watch.sh $CLAUDE_CODE_SESSION_ID "$(pwd)" claude-code`
- description: `agmsg inbox stream`
- persistent: true

Then continue with the user's subcommand. This catches the case where the user invokes `/ag` as the first prompt before the SessionStart-hook directive has been acted on.

**Sandbox compatibility.** When Claude Code's sandbox is enabled, `watch.sh` (monitor mode) runs inside the sandbox and needs to write pidfiles and SQLite WAL files under `~/.agents/skills/ag/`. If monitor mode fails with write/permission errors there, add an allowlist entry to `~/.claude/settings.json` (or project-level `.claude/settings.local.json`):
...
1. **IMMEDIATELY** run inbox check for each TEAM: `~/.agents/skills/ag/scripts/inbox.sh $TEAM $AGENT`
2. Do NOT ask the user what to do — just run the inbox check.
3. If there are messages, read and respond appropriately. To reply:
   `~/.agents/skills/ag/scripts/send.sh $TEAM $AGENT <to_agent> "<message>"`

If argument is "history":
1. Run: `~/.agents/skills/ag/scripts/history.sh $TEAM $AGENT`

If argument is "team":
1. For each TEAM, run: `~/.agents/skills/ag/scripts/team.sh $TEAM`
...
2. Run `~/.agents/skills/ag/scripts/identities.sh "$(pwd)" claude-code` to see whether the role is already registered for this (project, type).
3. If the name does not appear in the output, join under the existing team. Read TEAMS from the in-session whoami state (it may be a single team or comma-separated). For a single team, run `~/.agents/skills/ag/scripts/join.sh <team> <name> claude-code "$(pwd)"`. For multiple teams, ask the user which team to join the new role into, then run join.sh for that team.
4. **Pre-flight claim** the actas exclusivity lock so this role isn't already owned by another live session: `~/.agents/skills/ag/scripts/actas-claim.sh "$(pwd)" claude-code <name> "$CLAUDE_CODE_SESSION_ID"`. Read the `status=` line of the output:
    - `status=ok ...`: proceed to step 5.
    - `status=held team=<team> owner=<sid>`: another live session currently owns `<name>` in `<team>`. Tell the user: "Cannot actas as `<name>` — it is held by session `<sid>` in team `<team>`. Run `/ag drop <name>` in that session first, then retry." Then abort — do NOT touch the running Monitor.
...
2. Run `~/.agents/skills/ag/scripts/reset.sh "$(pwd)" claude-code <name> "$CLAUDE_CODE_SESSION_ID"` to remove only that role's registration for this project. If the role has no other registrations left, reset.sh also drops it from the team config. The 4th argument releases any actas exclusivity locks this session held on the role so peers can pick it up immediately (see #62).
...
2. Run: `~/.agents/skills/ag/scripts/spawn.sh <type> <name> --project "$(pwd)" [options]`
   - spawn.sh pre-joins `<name>`, then opens a tmux pane/window (when this session is inside tmux) or a new OS terminal, and launches the target CLI with `/ag actas <name>` as its initial prompt.
...
2. Determine which team `<name>` belongs to (as with `send`), then run:
   `~/.agents/skills/ag/scripts/despawn.sh <team> $AGENT <name> [--force] [--timeout <secs>]`
...
1. Run: `~/.agents/skills/ag/scripts/delivery.sh status claude-code "$(pwd)"`
2. Show the output to the user.

If argument starts with "mode" followed by a mode name (e.g. "mode monitor"):
1. Parse the mode (one of `monitor`, `turn`, `both`, `off`).
2. Run: `~/.agents/skills/ag/scripts/delivery.sh set <mode> claude-code "$(pwd)"`
3. Read the `AGMSG-DIRECTIVE` block in the command output and follow it (invoke Monitor or TaskStop as instructed).

If argument is "hook on" (legacy alias):
1. Run: `~/.agents/skills/ag/scripts/delivery.sh set turn claude-code "$(pwd)"`
2. Tell the user: "Delivery mode set to 'turn' (legacy hook on behavior). Consider using /ag mode monitor for real-time push."

If argument is "hook off" (legacy alias):
1. Run: `~/.agents/skills/ag/scripts/delivery.sh set off claude-code "$(pwd)"`
2. Tell the user: "Delivery mode set to 'off'."

If argument is "config":
1. Run: `~/.agents/skills/ag/scripts/config.sh show`
2. Show the output to the user.

If argument starts with "config set" (e.g. "config set hook.check_interval 30"):
1. Parse key and value from the arguments.
2. Run: `~/.agents/skills/ag/scripts/config.sh set <key> <value>`

If argument is "version":
1. Run: `~/.agents/skills/ag/scripts/version.sh`
2. Show the output — the installed version (git-describe provenance recorded at install time).

If argument is "reset":
1. Run: `~/.agents/skills/ag/scripts/reset.sh "$(pwd)" claude-code`
